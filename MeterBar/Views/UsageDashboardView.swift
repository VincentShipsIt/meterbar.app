...
struct UsageDashboardView: View {
    ...
    private var overviewStatusDetail: String {
        ...
        if tightestLimit.percentLeft <= 0 {
            return "\(tightestLimit.title) is out until reset. Tracking \(providerSnapshots.count) local provider sources."
        }
        return "\(tightestLimit.title) has \(tightestLimit.percentLeft)% left. Tracking \(providerSnapshots.count) local provider sources."
    }
